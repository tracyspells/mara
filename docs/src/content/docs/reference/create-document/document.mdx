...
To create one, use [`Mara.createDocument()`](/reference/create-document).
...
- `makeChanges` - A function that is called to make changes to a document's contents. The function receives `contents` as the first and only argument.
    ...
    Unfortunately, nested tables aren't copied (as we're not deep copying the document contents). There are ways to mitigate this [issue](/reference/create-document/#updating-nested-state), however.
...
You must call `Mara.trackDocumentHistory()` on your document before using this method!
